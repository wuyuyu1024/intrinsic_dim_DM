...
class NNinv_torch:
    ...
    def fit(self, X_2d, X, epochs=150, batch_size=128, verbose=0, early_stop=True, patience=5):
        ...
        if early_stop:
            ...
            for epoch in range(epochs):
                ...
                if verbose:
                    loss_per_epoch /= len(X_2d)
                    print(f'Epoch {epoch+1}/{epochs}, Loss: {loss_per_epoch:.8f}')
            print('Finished Training at epoch {}'.format(epoch+1))
        

    def transform(self, X_2d, batch_size=128, **kwargs):
        with torch.no_grad():
            # X_2d = torch.tensor(X_2d, dtype=torch.float32).to(self.device)
            # X_recon = self.model(X_2d)
